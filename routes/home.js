  ...
  if (record) {
    ...
    while (isUnique) {
      ...
      let result = Link.findOne({
        where: {
          shortUrl: `https://jacs-url-shortener.herokuapp.com/${code}`
        }
      })
...
router.get('/:id', (req, res) => {
  Link.findOne({
    where: {
      shortUrl: `https://jacs-url-shortener.herokuapp.com/${req.params.id}`
    }
  }).then((link) => {
    return res.redirect(`${link.longUrl}`)
  }).catch((e) => {
    let warning_msg = `The short link "https://jacs-url-shortener.herokuapp.com/${req.params.id}" is invalid!`
